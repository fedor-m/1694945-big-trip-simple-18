...
const createFormCreateTemplate = (point, destination, offers) => {
  ...
  const picturesList = [];
  const offersList = [];
  const allOffers = OFFERS.filter((o)=>o.id > 0);
  ...
  for (const d of DESTINATIONS) {
    destinationsList.push(`<option value="${d.name}"></option>`);
  }
  for (const p of pictures) {
    picturesList.push(`<img class="event__photo" src="${p.src}" alt="Event photo"></img>`);
  }
  for (const o of allOffers)
  {
    const s = offers.find((item)=>item.id === o.id && item.id !== 0) ? 'checked=""' : '';
    offersList.push(`
          <div class="event__offer-selector">
            <input class="event__offer-checkbox  visually-hidden" id="event-${o.title. replaceAll(' ', '-').toLowerCase()}-1" type="checkbox" name="${o.title.replaceAll (' ', '-').toLowerCase()}" ${s} >
            <label class="event__offer-label" for="${o.title.replaceAll(' ', '-').toLowerCase ()}-1">
